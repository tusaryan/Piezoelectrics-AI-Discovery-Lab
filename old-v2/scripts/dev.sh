...
print_summary() {
    ...
    if [ $exit_code -eq 0 ] && [ ${#SETUP_ERRORS[@]} -eq 0 ]; then
        if [ ${#SETUP_WARNINGS[@]} -eq 0 ]; then
            ...
            echo -e "${GREEN}All configurations, dependencies, ML components, and databases are successfully scaled.${NC}"
        else
            echo -e "${YELLOW}⚠️ INSTALLATION COMPLETED, BUT WITH WARNINGS:${NC}"
            for w in "${SETUP_WARNINGS[@]}"; do echo -e "   -> $w"; done
        fi
    else
        echo -e "${RED}❌ INSTALLATION FAILED OR ENCOUNTERED ERRORS:${NC}"
        if [ $exit_code -ne 0 ]; then
            echo -e "   -> A core installer command failed causing deployment trap to cleanly halt (Exit Code: $exit_code)."
        fi
        for e in "${SETUP_ERRORS[@]}"; do echo -e "   -> $e"; done
        if [ ${#SETUP_WARNINGS[@]} -gt 0 ]; then
            echo -e "${YELLOW}Previous Warnings Logged:${NC}"
            for w in "${SETUP_WARNINGS[@]}"; do echo -e "   -> $w"; done
        fi
        echo -e "${CYAN}Please review the explicit failures above, and attempt manual fixes or retry ${NC}bash scripts/dev.sh setup${CYAN}.${NC}"
    fi
    echo "=========================================================================="
}
...
cmd_setup() {
    ...
    # 2. Check Node version and Install JS dependencies
    log "[Step 2/5] Checking Node version and installing dependencies (pnpm)..."
    ...
    if [ "$NODE_MAJOR" -lt "$REQUIRED_NODE_MAJOR" ] || ([ "$NODE_MAJOR" -eq "$REQUIRED_NODE_MAJOR" ] && [ "$NODE_MINOR" -lt "$REQUIRED_NODE_MINOR" ]); then
        err "You are using Node.js $NODE_VERSION. Node.js version >= 20.9.0 is required."
        exit 1
    fi
    ...
    log "[Step 3.5/5] Installing heavy ML backends (PySR/Julia directly to local venv)..."
    python3 -c "import pysr; pysr.install()" || warn "PySR local Julia backend installation failed."
    success "PySR Julia backend initialized natively"
...
cmd_clean() {
    ...
    # Graceful Turborepo workspace handling
    if command -v npx &> /dev/null && [ -f "node_modules/.bin/turbo" ]; then
        log "Triggering Turborepo workspace cleanup protocol..."
        npx turbo clean 2>/dev/null || warn "Turbo clean encountered an issue, proceeding with deep purge anyway."
    else
        warn "Turbo executable not found locally; bypassing workspace scripts and executing direct aggressive purge."
    fi
    
    # Node
    rm -rf node_modules apps/web/node_modules apps/web/.next .turbo .pnpm-store
    success "Removed node_modules, .next, .turbo, and .pnpm-store caches"
    
    # Python
    find . -type d -name "__pycache__" -exec rm -rf {} + 2>/dev/null || true
    find . -type f -name "*.pyc" -exec rm -f {} + 2>/dev/null || true
    rm -rf .venv apps/api/.venv
    success "Removed __pycache__ and all .venv environments"
    
    # Build artifacts
    rm -rf apps/api/api.egg-info packages/*/dist packages/*/*.egg-info build dist
    success "Removed egg-info and build directories"
    
    log "Clean complete. Run 'bash scripts/dev.sh setup' to reinstall."
}

cmd_clean_data() {
    log "Cleaning data artifacts (models, datasets, generated files)..."
    warn "This will remove all locally trained models and uploaded CSV files in 'data/'."
    read -p "Are you sure? (y/N) " -n 1 -r
    echo
    if [[ ! $REPLY =~ ^[Yy]$ ]]; then
        log "Aborted data clean."
        return
    fi
    
    rm -rf data/models/* data/uploads/* data/exports/* 2>/dev/null || true
    # Restoring directory structures
    mkdir -p data/models data/uploads data/exports
    touch data/models/.gitkeep data/uploads/.gitkeep data/exports/.gitkeep
...
cmd_start() {
    ...
    if [ "$NODE_MAJOR" -lt "$REQUIRED_NODE_MAJOR" ] || ([ "$NODE_MAJOR" -eq "$REQUIRED_NODE_MAJOR" ] && [ "$NODE_MINOR" -lt "$REQUIRED_NODE_MINOR" ]); then
        err "You are using Node.js $NODE_VERSION. For Next.js, Node.js version >= 20.9.0 is required."
        exit 1
    fi
...
case "${1:-}" in
    ...
    *)
        ...
        echo "  clean       Remove node_modules, .next, __pycache__, .venv, .turbo"
        echo "  clean:data  Remove all generated ML models, datasets, and exports in data/"
